            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-dialog-custom modal-xl" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="provincia">Provincia:</label>
                                <select class="form-control" id="provinciaedit" name="provincia">
                                    @foreach($provinces as $province)
                            ...
                            <div class="form-group">
                                <label for="distrito">Distrito:</label>
                                <select class="form-control" id="distritoedit" name="distrito" disabled>
                                    <option value="">Seleccione el distrito</option>
                                    <!-- Districts will be dynamically populated based on the selected province -->
                                </select>
                            </div>
...
function capturar(id,lugar,provincia,distrito,fecha,diahabiles,tipo)
    ...
    $(document).ready(function () {
        // Evento que se activa cuando se selecciona una provincia
        $('#provincia').change(function () {
            // Obtiene el ID de la provincia seleccionada
            var provinciaId = $(this).find(':selected').attr('id').replace('provincia_', '');

            // Realiza una petición AJAX para obtener los distritos
            $.ajax({
                type: 'GET',
                url: '/get-districts/' + provinciaId,
                success: function (data) {
                    // Limpia el dropdown de distritos
                    $('#distrito').empty();

                    // Agrega los nuevos distritos al dropdown
                    $.each(data, function (key, value) {
                        $('#distrito').append('<option value="' + value.id + '">' + value.distrito + '</option>');
                    });

                    // Habilita el dropdown de distritos
                    $('#distrito').prop('disabled', false);
                },
                error: function (xhr, textStatus, errorThrown) {
                    console.error('Error al obtener los distritos:', errorThrown);
                }
            });
        });
    });
